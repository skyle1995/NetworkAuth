...
- **多开与顶号**：登录并发数受**有效多开** = 应用 `multi_open_count` + **会员等级额外多开** 控制；「一个开」的计数单位由 `multi_open_scope` 决定（0 单设备/1 单IP/2 每次登录）。同一个「开」重登先清掉自己的旧会话（不占新名额）；不同「开」未达上限则各自在线（非顶号）；**达到上限后**才按登录方式处理——顶号踢掉最早的「开」，非顶号拒绝并返回「已达最大同时在线数」。即会员额外多开只抬高上限，不绕过顶号/非顶号规则。
